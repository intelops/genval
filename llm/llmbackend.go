...
func generateOpenAIOptions(c *RequirementSpec) ([]openai.Option, error) {
	...
	for _, openAIConfig := range c.LLMSpec.OpenAIConfig {
		...
		return nil, fmt.Errorf("Model and APIKey must be specified :%v", err)
	}
	return opts, nil
}
